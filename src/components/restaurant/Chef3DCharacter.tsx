...
// Universally accessible 3D Master Chef character render for localhost and production
...
export function Chef3DCharacter({
  ...
  useEffect(() => {
    ...
    // If a non-http URL is passed that localhost cannot decode, fallback safely
    const isCustomScheme =
      imageSrc.startsWith("dyad-media://") ||
      (!imageSrc.startsWith("http") &&
        !imageSrc.startsWith("/") &&
        !imageSrc.startsWith("data:"));

    const resolvedSrc = isCustomScheme ? DEFAULT_CHEF_IMAGE : imageSrc;
    ...
    img.onload = () => {
      ...
      try {
        ...
        // Threshold extraction with feathering
        ...
        // Fallback for CORS or canvas read limitations
        setHasError(true);
      }
    };
  ...
  const currentDisplaySrc =
    imageSrc.startsWith("dyad-media://") ? DEFAULT_CHEF_IMAGE : imageSrc;
    ...
    >
      ...
      <div className="relative flex items-center justify-center">
        ...
        <canvas
          ref={canvasRef}
          className={cn(
            "w-[290px] sm:w-[370px] md:w-[440px] h-auto object-contain transition-all duration-500 will-change-transform transform-gpu",
        ...
        {/* Fallback Direct Render with high contrast styling */}
        {(!isCutoutReady || hasError) && (
          <div className="relative w-[280px] sm:w-[360px] md:w-[420px] aspect-square rounded-full overflow-hidden flex items-center justify-center border border-orange-500/30 bg-neutral-950/80 shadow-[0_20px_50px_rgba(0,0,0,0.9)]">
            <img
      ...
      {/* 3. Multi-Tiered Depth Contact Shadows */}
