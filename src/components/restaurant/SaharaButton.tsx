...
export function SaharaButton({
  ...
  const sizeClasses = {
    ...
    lg: "px-12 sm:px-14 py-3.5 sm:py-4 text-base",
  };
  ...
  return (
    <button
      ...
      className={cn(
        "bg-gradient-to-r from-red-500 to-orange-500 rounded-full shadow-xl group-hover:shadow-2xl group-hover:shadow-red-600 shadow-red-600 uppercase font-serif tracking-widest relative overflow-hidden group text-transparent cursor-pointer z-10",
        "after:absolute after:rounded-full after:bg-red-200 after:h-[85%] after:w-[95%] after:left-1/2 after:top-1/2 after:-translate-x-1/2 after:-translate-y-1/2",
        "hover:saturate-[1.15] active:saturate-[1.4] transition-all select-none",
        sizeClasses[size],
        className
      )}
      {...props}
    >
      {/* Invisible anchor text to maintain natural button layout */}
      <span className="opacity-0 pointer-events-none select-none inline-block">
        {primaryText.length > hoverText.length ? primaryText : hoverText}
      </span>

      {/* Slide-Up Primary Text */}
      <p className="absolute z-40 font-semibold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent top-1/2 left-1/2 -translate-x-1/2 group-hover:-translate-y-full h-full w-full transition-all duration-300 -translate-y-[30%] tracking-widest flex items-center justify-center pointer-events-none">
        {primaryText}
      </p>

      {/* Slide-In Hover Text */}
      <p className="absolute z-40 top-1/2 left-1/2 bg-gradient-to-r from-red-700 to-orange-700 bg-clip-text text-transparent -translate-x-1/2 translate-y-full h-full w-full transition-all duration-300 group-hover:-translate-y-[40%] tracking-widest font-extrabold flex items-center justify-center pointer-events-none">
        {hoverText}
      </p>

      {/* Multi-layered Sahara Dunes Gradient SVG */}
      <svg
      ...
      >
        <defs>
          <linearGradient id={`sssurf-grad-${gradId}`} y2="100%" x2="50%" y1="0%" x1="50%">
            <stop offset="0%" stopOpacity="1" stopColor="hsl(37, 99%, 67%)" />
            <stop offset="100%" stopOpacity="1" stopColor="hsl(316, 73%, 52%)" />
          </linearGradient>
        </defs>
        <g transform="matrix(1,0,0,1,0,-91.0877685546875)" fill={`url(#sssurf-grad-${gradId})`}>
          <path
            opacity="0.05"
      ...
      {/* Red Wave Horizon Silhouette SVG */}
      <svg
        className="absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-[30%] group-hover:-translate-y-[33%] group-hover:scale-95 transition-all duration-500 z-40 fill-red-500 pointer-events-none"
